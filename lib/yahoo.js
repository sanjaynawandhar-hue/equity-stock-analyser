...
// Yahoo's public chart/search APIs return 200 to a plain UA but 429 to a full
// Chrome UA sent with Referer/Origin/Cookie (that combo looks like a scraper).
// So we deliberately use a minimal User-Agent and NO extra headers — this makes
// the DIRECT calls work from servers, no proxy needed.
...
// rate-limited. Used only for non-crumb endpoints (charts). A free JINA_API_KEY
// raises the per-IP rate limit substantially (the free-tier limit gets hit on
// a busy always-on server).
...
function directHeaders(sess) {
  // Minimal headers only — adding Referer/Origin/Cookie triggers Yahoo's 429.
  return {
    'User-Agent': UA,
    ...(sess && sess.crumb && sess.cookie ? { Cookie: sess.cookie } : {}),
  };
}
...
async function yfetch(pathAndQuery, { withCrumb = false, proxy = false } = {}) {
  ...
  // --- 1) Direct (works with the minimal UA above; no cookie needed for the
  //        public chart/search endpoints). ---
  for (let attempt = 0; attempt < (withCrumb ? 2 : 1); attempt++) {
    const sess = withCrumb ? await ensureSession(attempt > 0) : null;
...
/** Real name/sector/industry for a symbol via the search endpoint (works
 *  through the proxy — unlike quoteSummary it needs no crumb). */
async function assetInfo(symbol) {
  const bare = String(symbol).replace(/\.(NS|BO)$/, '').toUpperCase();
  const json = await yfetch(`/v1/finance/search?q=${encodeURIComponent(bare)}&quotesCount=8&newsCount=0&listsCount=0`, { proxy: true });
  const quotes = (json && json.quotes) || [];
  const match = quotes.find((q) => (q.symbol || '').toUpperCase() === String(symbol).toUpperCase())
    || quotes.find((q) => (q.symbol || '').replace(/\.(NS|BO)$/, '').toUpperCase() === bare)
    || quotes.find((q) => /\.(NS|BO)$/.test(q.symbol || ''));
...
module.exports = {
  chart, quoteSummary, financials, searchSymbols, assetInfo, ensureSession, SUMMARY_MODULES,
